...
- Terminal output parsing is allowed only as bounded PTY fallback evidence in the terminal session/runtime modules. Prefer structured Codex app-server evidence whenever available, use the scoped Codex notify bridge only to refresh PTY state after turn-complete events, and cover parser or side-channel changes with regression tests.
